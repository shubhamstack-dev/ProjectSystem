...
def create_process(body: dict = Body(...), caller: Caller = Depends(current_user)):
    ...
    if db.execute(select(M.Process).where(M.Process.name == name)).scalar_one_or_none():
        raise HTTPException(409, f"A process called {name} already exists")
    p = M.Process(name=name, code=(body.get("code") or "").strip() or None,
...
def update_process(pid: int, body: dict = Body(...), caller: Caller = Depends(current_user)):
    ...
    if "name" in body:
        ...
        clash = db.execute(select(M.Process).where(
            M.Process.name == nm, M.Process.id != pid)).scalar_one_or_none()
        if clash:
